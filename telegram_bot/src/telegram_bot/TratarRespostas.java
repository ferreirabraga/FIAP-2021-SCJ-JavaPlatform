        ...
        switch (texto) {
            ...
            case "SIM_OPEN_WEATHER":
                if (ultimaInteracao.equals(OpcoesEnum.OPEN_WEATHER.name())) {
                    ...
                    setResposta(String.format("O tempo de %s Ã© %s e a temperatura de %s", propHistorico.getProperty("cidade"), weatherList.get(0).getDescription(), temperatura + "Â°C\n"
            ...
            default:
                if (ultimaInteracao.equals(OpcoesEnum.CRIPTO.getOpcao()) || ultimaInteracao.equals(OpcoesEnum.SIM_CRIPTO.getOpcao())) {
                    ...
                    if (null == ow) {
                        ...
                        List<Weather> weatherList = ow.getWeather();
                        setResposta(String.format("O tempo de %s Ã© %s e a temperatura de %s", texto, weatherList.get(0).getDescription(), ow.getMain().getTemp() + "Â°C\n"
                                + medeTemperatura(ow.getMain().getTemp()))
                                + "\nDeseja ver o clima de outra cidade?");
                        addSegundaInteracaoWeather();
                    ...
                    if (getResposta().length() == 0) {
                        ...
                        switch (confereExpressao) {
                            ...
                            case "Start":
                                ...
                                break;
                            default:
                                setResposta("Joinha? Para iniciarmos nossa conversa me enviando a opção  /start ou seu nome Belezinha?. ");
